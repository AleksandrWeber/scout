...
const generateWithGemini = async (input: ReportBuildInput): Promise<ExecutiveNarrative | null> => {
  if (!GEMINI_API_KEY) {
    return null;
  }

  const prompt = buildExecutiveReportPrompt(input);
...
const generateWithOpenAi = async (input: ReportBuildInput): Promise<ExecutiveNarrative | null> => {
...
export const generateExecutiveNarrative = async (
  ...
  try {
    if (provider === 'gemini') {
      const narrative = await generateWithGemini(normalizedInput);
      if (narrative) {
    ...
    if (provider === 'openai') {
      const narrative = await generateWithOpenAi(normalizedInput);
      if (narrative) {
